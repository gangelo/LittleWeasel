  ...
  describe '#validate_word_preprocessors' do
    context 'when passing a blank Array' do
      it 'passes validaton' do
        expect { subject.validate_word_preprocessors(word_preprocessors: []) }.to_not raise_error
      end
    end

    context 'when passing a an Array with valid word preprocessors' do
      it 'passes validation' do
        expect { subject.validate_word_preprocessors(word_preprocessors: [MockWordPreprocessor.new]) }.to_not raise_error
      end
    end
